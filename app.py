...
def hent_data():
    ...
    try:
        headers = {'User-Agent': 'SkuldelevApp/6.0'}
...
with st.sidebar:
    ...
    st.session_state.bud_elkedel = st.number_input("Elkedel bud (Nedreg)", value=float(st.session_state.bud_elkedel))
    st.session_state.bud_motor = st.number_input("Motor bud (Opreg)", value=float(st.session_state.bud_motor))
...
if not el_df.empty:
    ...
    if not el_frem.empty:
        ...
        c_eco3.info(f"**Spotpris nu:** {round(el_frem['SpotPriceDKK'].iloc[0])} kr")

        # Opbyg graf-data eksplicit
        el_frem['Tidspunkt'] = el_frem['Tid'].dt.strftime('%d/%m %H:%M')
        chart_data = el_frem.set_index('Tidspunkt')[['SpotPriceDKK']].copy()
        chart_data['Elkedel Grænse'] = float(st.session_state.bud_elkedel)
        chart_data['Motor Grænse'] = float(st.session_state.bud_motor)
...
# --- 7. HURTIGTJEK ---
st.divider()
st.header("Hurtigtjek: mFRR Varighed lige nu")
if not vejr_df.empty:
