...
def hent_data():
    ...
    try:
        ...
        r_v = requests.get(url_v, headers={'User-Agent': 'SkuldelevApp/13.0'}, timeout=5).json()
        rows = []
...
# --- 3. SIDEBAR MED INFO-BOKSE ---
with st.sidebar:
    st.header("Drifts-kontrol")
    st.session_state.tank_pct = st.slider("Aktuel Tank %", 0, 100, st.session_state.tank_pct)
    
    st.divider()
    # Budindtastning med info-bokse direkte under
    st.session_state.bud_el = st.number_input("Bud Elkedel (DKK)", value=float(st.session_state.bud_el))
    t_kedel = len(el_df[el_df['SpotPriceDKK'] <= st.session_state.bud_el])
    st.info(f"💡 Giver **{t_kedel} timer** kørsel")
    
    st.session_state.bud_mo = st.number_input("Bud Motor (DKK)", value=float(st.session_state.bud_mo))
    t_motor = len(el_df[el_df['SpotPriceDKK'] >= st.session_state.bud_mo])
    st.warning(f"💡 Giver **{t_motor} timer** kørsel")
...
for _, row in prog.iterrows():
    ...
    # Beregn time-step uden savtakker
    beholdning = beholdning + (bio - aftag)/1000
    beholdning = max(0, min(TANK_A_MAX_MWH, beholdning))
